...
def merge_call_reports(
    ...
    *,
    ...
    for source in (sanas, bvc):
        for label, data in (source.get("engines") or {}).items():
            engines[label] = data

    skipped = {
        **(main.get("skipped_engines") or {}),
        **(sanas.get("skipped_engines") or {}),
        **(bvc.get("skipped_engines") or {}),
    }
    return {
        "call_id": call_id,
        "stt": main.get("stt") or sanas.get("stt") or bvc.get("stt"),
        "engines": engines,
        "skipped_engines": skipped,
        "reference_turns": main.get("reference_turns")
        or sanas.get("reference_turns")
...
def merge_run_directory(run_dir: Path) -> dict:
    ...
    for tier_a in run_dir.glob("*_tier_a_wer_report.json"):
        ...
        tier_b = run_dir / f"{call_id}_tier_b_wer_report.json"
        tier_c = run_dir / f"{call_id}_tier_c_wer_report.json"
        per_call[call_id] = merge_call_reports(
            call_id, tier_a=tier_a, tier_b=tier_b if tier_b.exists() else None, tier_c=tier_c if tier_c.exists() else None
        )
